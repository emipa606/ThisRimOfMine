...
internal class SketchResolver_HouseEdge_R5_Laundrette : SketchResolver
{
    ...
    protected override void ResolveInt(ResolveParams parms)
    {
        ...
        if (Rand.Bool)
        {
            sketch.AddThing(stoolDef, new IntVec3(3, 0, 1), Rot4.East);
        }

        if (Rand.Bool)
        {
            sketch.AddThing(stoolDef, new IntVec3(3, 0, 2), Rot4.East);
        }

        if (Rand.Bool)
        {
            sketch.AddThing(stoolDef, new IntVec3(3, 0, 3), Rot4.East);
        }

        if (Rand.Bool)
        {
            sketch.AddThing(stoolDef, new IntVec3(3, 0, 4), Rot4.East);
        }

        if (Rand.Bool)
        {
            sketch.AddThing(stoolDef, new IntVec3(3, 0, 5), Rot4.East);
        }
